---
source_url: https://docs.lightly.ai/train/stable/settings/train_settings.html
---

# Train SettingsÂ¶

This page covers the settings available for training tasks like object detection and segmentation in LightlyTrain. For settings related to pretraining and distillation, please refer to the [Pretrain & Distill Settings](settings__pretrain_settings.md#pretrain-settings) page.
...
`data` | `dict`  
`str` | â | Dataset configuration dict, or path to a YAML file containing the dataset configuration.  
`model` | `str`  
`Path` | â | Model identifier (e.g. `"dinov3/vitt16-ltdetr-coco"`) or path to a local checkpoint to fine-tune from.  
`model_args` | `dict` | `None` | Task/model-specific training hyperparameters.  
`steps` | `int` | `"auto"` | Number of training steps. `"auto"` selects a model-dependent default.  
`precision` | `str` | `"bf16-mixed"` | Numeric precision mode (e.g. `"16-true"`, `"32-true"`, `"bf16-mixed"`).  
`batch_size` | `int` | `"auto"` | Global batch size across all devices.  
...
`resume_interrupted` | `bool` | `False` | Resume an interrupted/crashed run from the same `out` directory, including optimizer state and current step. Do not change any training parameters when using this.  
...
`logger_args` | `dict` | `None` | Logger configuration dict. `None` uses defaults; keys configure or disable individual loggers.  
`transform_args` | `dict` | `None` | Data transform configuration (e.g. image size, normalization).  
`metric_args` | `dict` | `None` | Metric configuration dict. `None` uses defaults; keys configure or disable individual metrics.  
`save_checkpoint_args` | `dict` | `None` | Checkpoint saving configuration (e.g. save frequency).  
`torch_compile_args` | `dict` | `None` | Torch compile configuration dict. `None` uses defaults; keys configure or disable torch.compile options.  
...
Look for the `Resolved Args` dictionary in the `train.log` file in the output directory of your run to see the final settings that were applied. This will include any overrides, automatically resolved values, and model-specific settings that are not listed on this page.

## OutputÂ¶

### `out`Â¶

The output directory where the model checkpoints and logs are saved. Create a new directory for each run! LightlyTrain will raise an error if the output directory already exists unless the `overwrite` flag is set to `True`.
...
Dataset configuration. You can either provide a dictionary with dataset parameters or a path to a YAML file containing the dataset configuration. Each task (detection, segmentation, etc.) has different dataset requirements. Refer to the task documentation for details on the expected dataset structure and configuration options.
  ...
  * [Object Detection](object_detection.md#object-detection-data)

  * [Instance Segmentation](instance_segmentation.md#instance-segmentation-data)

  * [Panoptic Segmentation](panoptic_segmentation.md#panoptic-segmentation-data)

  * [Semantic Segmentation](semantic_segmentation.md#semantic-segmentation-data)
...
Global batch size across all devices. The batch size per device/GPU is computed as `batch_size / (num_devices * num_nodes)`. By default, `batch_size` is set to `"auto"`, which selects a model-dependent default value.
...
Model identifier (for example `"dinov3/vitt16-ltdetr-coco"`) or the path to a checkpoint or exported model file. LightlyTrain automatically downloads weights if needed.

To resume from a crashed or interrupted run, use the `resume_interrupted` setting instead of pointing `model` to a previous checkpoint. This ensures that optimizer state and training progress are restored correctly.
...
`str`  
`None` | Path to backbone weights to load.  
`metric_log_classwise` | `bool` | Whether to log class-wise metrics.  
`scheduler_name` | `str` | Scheduler mode for LTDETR training.  
`scheduler_flat_steps` | `int`  
`"auto"` | Flat-cosine flat phase boundary.  
`scheduler_no_aug_steps` | `int`  
`"auto"` | Flat-cosine final tail length.  
...
Path to a checkpoint or exported model containing backbone weights to load before training. This enables loading custom pretrained weights. See [Pretrain & Distill](https://docs.lightly.ai/train/stable/pretrain_distill/index.html#pretrain-distill) for more details on pretraining on unlabeled data.
...
The backbone weights argument is ignored when loading an existing checkpoint via the `model` argument:
...
The backbone weights are only loaded when training starts from scratch using a model identifier without a dataset suffix (e.g. `-coco`, `-cityscapes`, etc.).

#### `metric_log_classwise`Â¶

If set to `True`, class-wise metrics (for example AP per class) are logged during validation. Default is `False` to reduce logging overhead. Not all models support this feature.
...
Any other value raises a `ValueError`. The legacy `scheduler` key is no longer accepted. For `"flat-cosine"`, `scheduler_flat_steps` and `scheduler_no_aug_steps` default to `"auto"` and are resolved from the LTDETR setup and the configured `total_steps`. Override them only when you need custom phase boundaries.
...
Total number of training steps. The default is `"auto"`, which selects a model-dependent value. Reduce for shorter training or increase to train longer.
...
There are two ways to continue training from a previous run:
  ...
  1. Resume an interrupted/crashed run and finish training with the same parameters.
  ...
  2. Load a checkpoint from a previous run and fine-tune with different parameters.
...
### Load Checkpoint for a New RunÂ¶

To continue training from a previous run but change training parameters (for example to train for more steps), set the `model` argument to the path of an exported model from a previous run and specify a new `out` directory. This way, training starts fresh but initializes weights from the provided checkpoint.

We recommend using the exported best model weights from `out/my_experiment/exported_models/exported_best.pt` for this purpose.
...
LightlyTrain saves two types of checkpoints during training:
  ...
  2. `out/my_experiment/exported_models`: Lightweight exported models containing only model weights. Used for inference and any further fine-tuning.
...
### `save_checkpoint_args`Â¶

Settings to configure checkpoint saving behavior. By default, LightlyTrain saves `last.ckpt` and `best.ckpt`. The best checkpoint is selected based on the `watch_metric` defined in `metric_args`.
...
`save_best` | `bool` | Track the best-performing checkpoint according to `watch_metric` in `metric_args`.  
  
#### `save_every_num_steps`Â¶

Number of training steps between each checkpoint save. Default is `1000`. Decrease to save more frequently. Too frequent saving may slow down training.
...
If set to `True`, the latest checkpoint and exported model (`last.ckpt` and `exported_last.pt`) are saved at each save interval. Default is `True`. Disable only when storage space is limited.
...
If set to `True`, the best-performing checkpoint and exported model (`best.ckpt` and `exported_best.pt`) are tracked and saved based on the validation metric defined by `watch_metric` in `metric_args`. Default is `True`.
...
`log_every_num_steps` | `int`  
`"auto"` | Training step interval for logging training metrics.  
`val_every_num_steps` | `int`  
`"auto"` | Training step interval that triggers a validation run.  
`val_log_every_num_steps` | `int`  
`"auto"` | Validation step interval for logging validation metrics.  
...
#### `log_every_num_steps`Â¶

Controls how frequently training metrics are written. Is set to `"auto"` by default, which selects a value based on the dataset size. Decrease the value for more frequent updates.
...
#### `val_every_num_steps`Â¶

Controls how frequently validation is performed during training. Is set to `"auto"` by default, which selects a value based on the dataset size. `"auto"` validates at least once every 1000 steps. Decrease the value to validate more often.
...
#### `val_log_every_num_steps`Â¶

Controls how frequently progress is logged during validation runs. Is set to `"auto"` by default, which selects a value based on the dataset size.
...
Check the respective task pages for the default transforms applied:
  ...
  * [Object Detection](object_detection.md#object-detection-transform-args)

  * [Instance Segmentation](instance_segmentation.md#instance-segmentation-transform-args)

  * [Panoptic Segmentation](panoptic_segmentation.md#panoptic-segmentation-transform-args)

  * [Semantic Segmentation](semantic_segmentation.md#semantic-segmentation-transform-args)
...
The metrics logged during training depend on the task. LightlyTrain computes task-appropriate metrics such as mAP for detection or mIoU for segmentation. Use `metric_args` to control which metrics are computed and logged.
...
Key | Type | Description  
---|---|---  
`classwise` | `bool` | Enable classwise metrics. If `False`, only aggregate metrics are logged.  
`train` | `bool` | Enable train metrics. If `False`, only validation metrics are logged.  
`watch_metric` | `str` | Validation metric name monitored when selecting the best checkpoint.  
  
In addition to these global options, any metric name (for example `accuracy`, `f1`, `precision`, `recall`, or task-specific metrics such as mAP/mIoU variants) can also be used as a key in `metric_args`. Such per-metric keys accept a configuration dictionary as their value, or `None` to disable computation of that metric.

Check the `train.log` file for all available metrics for your task and model. For example, for classification the default metrics configuration looks like this:
...
To customize the metrics configuration, set the `metric_args` dictionary with the desired values. For example, to calculate top-3 accuracy set:
...
All values that are lists, for example the `topk` or `average` parameters, can contain multiple entries as shown above.
...
If set to `True`, per-class metrics (for example AP per class) are logged in addition to aggregate metrics. Default is `False` to reduce logging overhead.
...
If set to `True`, metrics are also computed and logged for the train set. By default, only metrics for the validation set are logged to reduce overhead.
...
Validation metric used to determine the best checkpoint when `save_best` is `True`. The default metric depends on the selected model.
...
### `torch_compile_args`Â¶

Dictionary to configure model compilation with [`torch.compile`](https://docs.pytorch.org/docs/stable/generated/torch.compile.html). All entries in the dictionary are passed directly to `torch.compile`. By default, model compilation is disabled for most models.
