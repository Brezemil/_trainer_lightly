---
source_url: https://docs.lightly.ai/train/stable/embed.html
---

# EmbedÂ¶

The `embed` command is used to embed images with a [training checkpoint](https://docs.lightly.ai/train/stable/pretrain_distill/index.html#train-output). An example command looks like this:
...
The above code example pretrains a model and uses the last training checkpoint to generate image embeddings from the images in `"my_data_dir"`.

Tip

See [`lightly_train.embed()`](python_api__lightly_train.md#lightly_train.embed "lightly_train.embed") for a complete list of arguments.
...
The `data` argument specifies the directory with images to embed. The same image formats as in the [training command](https://docs.lightly.ai/train/stable/pretrain_distill/index.html#pretrain-data) are supported.

## CheckpointÂ¶

The `checkpoint` argument specifies the Lightly**Train** checkpoint to use for embedding. This is the checkpoint saved to `out/my_experiment/checkpoints/last.ckpt` after training.
...
The `format` argument specifies the format in which the embeddings are saved. The following formats are supported:
  ...
  * `lightly_csv`: Embeddings are saved as a CSV file with one row per image. The CSV file is compatible with the [Lightly Worker](https://docs.lightly.ai/docs/custom-embeddings). The labels column is always set to 0. An example CSV file looks like this:
