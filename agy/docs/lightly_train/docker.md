---
source_url: https://docs.lightly.ai/train/stable/docker.html
---

# DockerÂ¶

Lightly**Train** is available as a Docker image on [Docker Hub](https://hub.docker.com/r/lightly/train) for containerized deployment.

## InstallationÂ¶

Install Docker with NVIDIA GPU support ([docs](https://docs.lightly.ai/docs/lightly-worker-installation-guide#install-docker-optionally-with-gpu-support)). Then, pull the latest version of the Lightly**Train** Docker image from Docker Hub:
...
Flags:
  ...
  * `-v /my_data_dir:/data`: Mounts the host directory `/my_data_dir` to the container directory `/data`. This directory must contain your training data. See the [Data](https://docs.lightly.ai/train/stable/pretrain_distill/index.html#pretrain-data) docs for more information on how to structure your data.




Once the container is running, you can run Lightly**Train** commands inside the container as if you had installed it locally. The only difference is that paths must be specified relative to the mounted directories `/out` and `/data`. For example, to pretrain a model, run the following command inside the container:
