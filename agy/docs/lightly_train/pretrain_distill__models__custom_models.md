---
source_url: https://docs.lightly.ai/train/stable/pretrain_distill/models/custom_models.html
---
...
Lightly**Train** supports pretraining custom models. This requires writing a small wrapper around your model to implement the necessary methods. The wrapper must be a subclass of `torch.nn.Module` and implement the following methods:
...
Returns architecture information so that methods like distillation can select the correct training recipe without relying on heuristics. Should return a dict with:
...
Some [SSL methods](https://docs.lightly.ai/train/stable/pretrain_distill/methods/index.html#methods) do not call the `forward_pool` method and only use the unpooled features. In this case, the embedding layer is applied directly to the output of `forward_features`.
