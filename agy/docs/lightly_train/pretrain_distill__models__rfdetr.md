---
source_url: https://docs.lightly.ai/train/stable/pretrain_distill/models/rfdetr.html
---
...
## Pretrain and Fine-tune an RF-DETR ModelÂ¶

### PretrainÂ¶

Pretraining RF-DETR models with LightlyTrain is straightforward. Below we provide the minimum scripts for pretraining using `rfdetr/rf-detr-base` as an example:
...
After pretraining, you can load the exported model for fine-tuning with RF-DETR. For now, RF-DETR only supports datasets in COCO JSON format. Below we provide the minimum scripts for fine-tuning using the [Coconuts dataset from Roboflow](https://universe.roboflow.com/traindataset/coconuts-plj8h/dataset/1/download/coco) in COCO JSON format:
...
which can be run with RF-DETRâs DDP training:
